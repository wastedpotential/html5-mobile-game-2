...
export function showComponent(pixiComponent, duration = 0.3, delay = 0, y = null, fadeIn = true) {
	const parent = pixiComponent.parent;
	const holder = new PIXI.Container(); //{resolution: devicePixelRatio, roundPixels: true});
	const alphaStart = fadeIn ? 0 : 1;
	const filter = new PIXI.filters.AlphaFilter(alphaStart);
